...
var Attraction, Behaviour, Collision, ConstantForce, EdgeBounce, EdgeBlink,EdgeLine, EdgeWrap, Euler, ImprovedEuler, Integrator, Particle, Physics, Random, Spring, Vector, Verlet, Wander, namespace,
...
EdgeBlink = (function(_super) {

  __extends(EdgeBlink, _super);

  function EdgeBlink(min, max) {
    this.min = min != null ? min : new Vector();
    this.max = max != null ? max : new Vector();
    EdgeBlink.__super__.constructor.apply(this, arguments);
  }

  EdgeBlink.prototype.apply = function(p, dt, index) {
    if (p.pos.x - p.radius < this.min.x) {
      p.pos.x = this.min.x + p.radius;
	  p.vel.x = -p.vel.x;
...
EdgeLine = (function(_super) {

  __extends(EdgeLine, _super);

  function EdgeLine(min, max) {
    this.min = min != null ? min : new Vector();
    this.max = max != null ? max : new Vector();
    EdgeLine.__super__.constructor.apply(this, arguments);
  }

  EdgeLine.prototype.apply = function(p, dt, index) {
    pos1 = p.pos.x+p.radius;
	pos2 = p.pos.x-p.radius;
	if (pos1 > 390 && pos1<400 && (p.pos.y > this.max || p.pos.y < this.min) && p.vel.x > 0) {
		p.vel.x = -p.vel.x	
	}
	else if (pos2 > 400 && pos2<410 && (p.pos.y > this.max || p.pos.y < this.min) && p.vel.x<0) {
		p.vel.x = -p.vel.x	
	};
  };
